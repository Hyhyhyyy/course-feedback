...
API 与校园服务采用兼容 `/v1/chat/completions` 的接口，音视频模式需要支持图片输入，并支持 JSON 输出或 JSON Schema 结构输出；返回结果均在本地再次校验结构。只提供文本能力的服务暂不满足音视频模式要求。服务实现差异需要实际联调，配置成功不等于模型调用成功。当前未配置或验证远程服务，真实试验使用本地 Qwen3.5-4B。
