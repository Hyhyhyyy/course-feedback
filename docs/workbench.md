...
课镜在页面中直接执行本地音视频转写、取帧和时间定位。产品界面不再设置外部转写导入入口，来源与借鉴关系保留在仓库ATTRIBUTION中。上游转写格式兼容仅保留为已有研究工具，不是当前教师操作步骤。

复盘依次阅读重点时间窗、六类反馈、全课分布、问题概述与词云，再查看原始弹幕、候选讲解与画面。时间窗按含问题弹幕条数排序，属于回看优先级，不构成教学质量等级。默认图表为实际线性数量，压缩高峰开关采用 log(1+数量)，始终保留实际数量提示。

单条依据可以确认、排除或标为待核查，教师备注保存到本机项目。复核不覆盖原始分类，也不自动改变原统计；两者在导出 JSON 中分开保存。独立 HTML 包含原始弹幕、模型分析占位、教师大纲原文、已提取的课程画面及教师备注；完整视频保留在本机工作台。
...
获取进度条按四个真实阶段计算完成数：课程信息、弹幕快照、音视频下载、媒体校验。下载时另显示当前文件的已下载字节和约百分比，总量未知时使用动态条；该数值不代表完整任务耗时百分比。只有终态成功才显示获取成功，部分成功和失败保留具体阶段说明。状态请求中断时显示结果暂不可确认，并允许重新检查后台任务。


当前展示调整：未连接正式分析模型时，分类、问题数量与概述、词云、积极与情绪分析、优先复盘和讲解内容分析统一显示【请先连接大模型以获取准确分析】。页面及HTML导出不再展示规则候选数值，页面JSON导出也移除这些候选字段；本机原始试跑文件保留供研究复现。真实弹幕总量、时间分布、原文、视频和画面继续可用。该占位文字不代表连接模型后自动获得已经验证的准确率。


## 教师教学大纲与综合复盘

教师通过“导入教师教学大纲”上传TXT、Markdown或DOCX，也可以粘贴原文后保存。文件最大5MB；系统提取原文段落、分配o0001等引用编号，保存文件名、SHA256和导入时间到当前课程的analysis/syllabus.json。TXT支持UTF-8及GB18030，DOCX读取正文与表格文字，不执行宏、不解压到文件系统、不猜测图片和公式内容。当前不支持PDF、扫描图和旧式DOC；教师应先转为支持的文本格式。解析失败会保留已有大纲。

大纲保存与课程素材处理相互独立，更新大纲无需重复下载或转写；报告GET和导出均读取当前大纲。教师原文与自动推断严格分开，段落数量不等于教学目标数量。

报告增加两块：课镜音视频复盘直接展示已提取画面和播放时间、转写草稿数量；“教学大纲×讲解表现×学生反馈”显示大纲原文，预留讲解与大纲对应、目标下的反馈线索、综合复盘及改进建议。未连接大模型时，这些推断字段统一显示指定占位。当前后端报告接口和JSON/HTML导出也移除未验证的规则候选；研究用原始试跑文件保留本机。

综合评价的预期依据是教师自己的目标与安排、实际音视频内容和可追溯弹幕。它用于检查讲解覆盖与反馈线索，不据大纲或弹幕推算实际掌握率，也不自动输出课程质量总分。接入模型后仍需验证段落与讲解的对应、问题归属及建议的证据支持。

```mermaid
flowchart TB
 A[课程链接] --> B[课镜获取视频、音轨与当次弹幕]
 B --> C[本地转写与定时取帧
形成带播放时间的内容记录]
 B --> D[原始弹幕及时间分布]
 E[教师导入或粘贴教学大纲] --> F[正文解析、原文段落编号
保存当前课程的大纲]
 C --> G[课镜报告：视频与画面回看]
 D --> G
 F --> H[教师原定目标与安排]
...
新增四项测试覆盖大纲原文保持、DOCX解析、无效文件拒绝以及报告层推断屏蔽，累计25项通过。无教师真实大纲时不填造示例教学目标；导入后直接显示教师原文。

2026-09-14 当前反三角函数课程整课实测：音视频时长2836.132秒，处理耗时179.994秒，生成1279段转写草稿与24张带播放时间的课堂画面。页面已确认显示上述实际数量。此结果验证采集后音视频处理和报告展示链路，不代表转写准确率或综合评价质量已经验证；尚无教师真实大纲，综合分析继续使用大模型占位提示。
